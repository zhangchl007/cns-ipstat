...
func main() {
	...
	result, warnings, err := v1api.Query(ctx, "histogram_quantile(0.99, sum(rate(etcd_disk_wal_fsync_duration_seconds_bucket[5m])) by (instance, le))", time.Now(), v1.WithTimeout(5*time.Second))
	handleError(err)
